...
class HipDeviceAttribute(IntEnum):
    ...
    hipDeviceAttributeConcurrentManagedAccess = auto()          # Device can coherently access managed memory concurrently with the CPU
    ...
    hipDeviceAttributeHostNativeAtomicSupported = auto()        # Link between the device and the host supports native atomic operations
    ...
    hipDeviceAttributeLuid = auto()                             # 8-byte locally unique identifier in 8 bytes. Undefined on TCC and non-Windows platforms
    ...
    hipDeviceAttributeMaxTexture2DLinear = auto()               # Maximum dimensions (width, height, pitch) of 2D textures bound to pitched memory.
    ...
    hipDeviceAttributePageableMemoryAccessUsesHostPageTables = auto() # Device accesses pageable memory via the host's page tables
    ...
    hipDeviceAttributeMaxRegistersPerBlock = auto()             # 32-bit registers available to a thread block. This number is shared
    ...
    hipDeviceAttributeSingleToDoublePrecisionPerfRatio = auto() # Cuda only. Performance ratio of single precision to double precision.
    ...
    hipDeviceAttributeTexturePitchAlignment = auto()            # Pitch alignment requirement for 2D texture references bound to pitched memory;
    ...
    hipDeviceAttributeMemoryPoolSupportedHandleTypes = auto()   # Supported handle mask for HIP Stream Ordered Memory Allocator
...
def get_hbm_size(device_id=None):
    ...
    hip_try(hip_runtime.hipMemGetInfo(
